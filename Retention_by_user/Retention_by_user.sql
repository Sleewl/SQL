...
cohort_sizes AS (
  ...
  FROM first_actions
  GROUP BY ads_campaign, start_date
),
...
daily_active AS (
  ...
  WHERE ua.time::date IN (
    fa.start_date, 
    fa.start_date + 1, 
    fa.start_date + 7
  )
  GROUP BY fa.ads_campaign, fa.start_date, ua.time::date
),
...
retention_calc AS (
  SELECT
    da.ads_campaign,
    da.start_date,
    (da.event_date - da.start_date) AS day_number,
    ROUND(
      CAST(da.active_users AS numeric)
      / cs.cohort_size
    , 2) AS retention
  FROM daily_active da
  JOIN cohort_sizes cs
    ON da.ads_campaign = cs.ads_campaign
   AND da.start_date   = cs.start_date
)
