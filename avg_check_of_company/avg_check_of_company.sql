WITH 
campaign_users AS (
  SELECT user_id, 'Кампания № 1' AS ads_campaign FROM (VALUES
  ...
  SELECT user_id, 'Кампания № 2' AS ads_campaign FROM (VALUES
...
user_first_order AS (
  SELECT cu.user_id, cu.ads_campaign, MIN(ua.time) AS reg_date
  FROM user_actions ua
  JOIN campaign_users cu USING(user_id)
  WHERE ua.action = 'create_order'
    AND ua.time >= '2022-09-01'
    AND ua.time <  '2022-09-08'
  GROUP BY cu.user_id, cu.ads_campaign
),

order_amounts AS (
  SELECT o.order_id, SUM(p.price) AS order_amount
  FROM orders o
...
first_week_orders AS (
  SELECT ufo.ads_campaign, ua.user_id, oa.order_amount
  FROM user_actions ua
  JOIN user_first_order ufo USING(user_id)
  JOIN order_amounts    oa  USING(order_id)
  WHERE ua.action = 'create_order'
    AND ua.time >= ufo.reg_date
    AND ua.time <  ufo.reg_date + INTERVAL '7 days'
),

user_avg AS (
  SELECT ads_campaign, user_id, AVG(order_amount) AS avg_per_user
...
SELECT
  ads_campaign,
  ROUND(AVG(avg_per_user)::numeric, 2) AS avg_check
FROM user_avg
